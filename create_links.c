...
void	binary_search_links(char *r1, char *r2, t_graph *graph, t_flags *flags)
{
	int			mid;
	t_link_list	*add;
	t_link		*add_link;
...
void	create_links_binary(t_graph *graph, char *name_coor, t_flags *flags)
{
	...
	while (flags->i <= flags->count_of_room)
		binary_search_links(split_link[0], split_link[1], graph, flags);
	flags->i = 0;
	flags->count_of_room = last;
	while (flags->i <= flags->count_of_room)
	{
		binary_search_links(split_link[1], split_link[0], graph, flags);
	}
